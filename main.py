...
def receita_aleatoria():
    ...
    with open("aleatorias.txt", "r", encoding="utf-8") as arquivo:
        linhas = arquivo.readlines()
        indices = [i for i, linha in enumerate(linhas) if linha.startswith(nome_receita)]
        
        if indices:
            indice_receita = random.choice(indices)  # Escolhe aleatoriamente um índice de receita
            print("Receita encontrada!\n")
            for i in range(indice_receita, len(linhas)):
                if linhas[i].strip() == '----------':  # Verifica se é uma linha de separador antes de imprimir
                    print("Linha de separador encontrada")
                    break
